      ...
      namespace :s_corr_cpp do
        namespace :object do
          namespace :reference_cppu do
            ...
            task :delete do |t|
              def query
                condition1 = Destination.s_corr_app.create_on(Destination.s_corr[:link].eq(Destination.s_corr_cpp[:corr]))
                ...
                source = Arel::Nodes::JoinSource.new(
                  Destination.s_corr_cpp, [
                    Destination.s_corr_cpp.create_join(Destination.s_corr, condition1),
                ...
                manager.from(source)
                manager.where(Destination.s_corr_cpp[:object].eq(Destination::SCorrCpp::REFERENCE_CPPU))
                manager.to_sql
              end
