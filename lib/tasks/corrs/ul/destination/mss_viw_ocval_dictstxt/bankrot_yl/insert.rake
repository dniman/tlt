          ...
          task :insert do |t|
            ...
            def query
              ...
              .where(Source.client_types[:name].eq('Юридическое лицо')
                .and(
                  Source.organisations[:isbankrot].not_eq(nil)
                  .or(Source.organisations[:isbankrot].not_eq('N'))
                )
              )
            end
