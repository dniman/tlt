        ...
        namespace :industry do
          ...
          task :insert do |t|
            def link_app_prop_query
              ...
              .where(Destination.mss_objcorr_props[:code].eq('INDUSTRY'))
            end
            ...
            def query
              ...
              industries_ids = Source.___ids.alias("industries_ids")

              Source.clients
              .project([
                Source.___ids[:___link].as("link_app_up"),
                Arel.sql("#{ link_app_prop }").as("link_app_prop"),
                industries_ids[:link].as("link_dict"),
              ])
              ...
              .join(Source.___industries, Arel::Nodes::OuterJoin).on(Source.___industries[:name].eq(Source.___ids[:___industry_name]))
              .join(industries_ids).on(industries_ids[:id].eq(Source.___industries[:id]).and(industries_ids[:table_id].eq(Source::Industries.table_id)))
              ...
              .where(Source.client_types[:name].eq('Юридическое лицо')
                .and(Source.___ids[:___industry_name].not_eq(nil))
              )
            end
