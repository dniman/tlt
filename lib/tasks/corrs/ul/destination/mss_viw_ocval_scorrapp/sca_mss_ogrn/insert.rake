          ...
          task :insert do |t|
            ...
            def query
              ...
              .project([
                ...
                Source.organisations[:ogrn].as("varchar"),
                Source.organisations[:date_ogrn].as("actual_date"),
              ])
              ...
              .join(Source.organisations).on(Source.organisations[:clients_id].eq(Source.clients[:id]))
              .where(Source.client_types[:name].eq('Юридическое лицо')
                .and(Source.organisations[:ogrn].not_eq(nil))
              )
            end
            ...
            begin
              ...
              condition =<<~SQL
                mss_viw_ocval_scorrapp.link_app_up = values_table.link_app_up
                  ...
                  and mss_viw_ocval_scorrapp.actual_date = values_table.actual_date
              SQL
              ...
              Source.execute_query(query.to_sql).each_slice(1000) do |rows|
                rows.each do |row|
                  insert << {
                    ...
                    varchar: row["varchar"],
                    actual_date: row["actual_date"].nil? ? nil : row["actual_date"].strftime("%Y%m%d"),
                  }
                end
