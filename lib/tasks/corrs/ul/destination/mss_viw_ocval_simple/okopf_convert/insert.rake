          ...
          task :insert do |t|
            ...
            def query
              ...
              .project([
                ...
                Source.orgforms[:fullname].as("varchar"),
              ])
              ...
              .join(Source.orgforms, Arel::Nodes::OuterJoin).on(Source.orgforms[:id].eq(Source.organisations[:orgform]))
              .where(Source.client_types[:name].eq('Юридическое лицо')
                .and(Source.organisations[:orgform].not_eq(nil))
              )
            end
