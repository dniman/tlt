  ...
  namespace :fl_pers do
    namespace :destination do
      namespace :s_corr_app do
        namespace :object do
          namespace :column_dict_pers_phone do
            ...
            task :insert do |t|
              def query
                ...
                .project([
                  Source.___ids[:link],
                  Source.privates[:phone].as("value"),
                  Arel.sql("#{ Destination::SCorrApp::COLUMN_DICT_PERS_PHONE }").as("object"),
                ])
                ...
                .where(Source.client_types[:name].eq('Физическое лицо')
                  .and(Source.privates[:phone].not_eq(nil))
                )
              end
