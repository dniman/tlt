  ...
  namespace :fl_pers do
    namespace :destination do
      namespace :s_corr_app do
        namespace :object do
          namespace :column_person_doc_whom do
            ...
            task :insert do |t|
              def query
                ...
                .project([
                  Source.___ids[:link],
                  Source.privates[:pasportgiven].as("value"),
                  Arel.sql("#{ Destination::SCorrApp::COLUMN_PERSON_DOC_WHOM }").as("object"),
                ])
                ...
                .where(Source.client_types[:name].eq('Физическое лицо')
                  .and(Source.privates[:pasportgiven].not_eq(nil))
                )
              end
              ...
              begin
                ...
                Source.execute_query(query.to_sql).each_slice(1000) do |rows|
                  rows.each do |row|
                    insert << {
                      link_up: row["link"],
                      value: row["value"].is_a?(Time) ? row["value"].strftime("%Y%m%d") : row["value"],
                      object: row["object"],
                    }
                  end
