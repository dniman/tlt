  ...
  namespace :fl_pers do
    namespace :destination do
      namespace :s_corr_app do
        namespace :object do
          namespace :column_person_doc_date do
            ...
            task :insert do |t|
              def query
                ...
                .project([
                  Source.___ids[:link],
                  Source.privates[:pasportdate].as("value"),
                  Arel.sql("#{ Destination::SCorrApp::COLUMN_PERSON_DOC_DATE }").as("object"),
                ])
                ...
                .where(Source.client_types[:name].eq('Физическое лицо')
                  .and(Source.privates[:pasportdate].not_eq(nil))
                )
              end
