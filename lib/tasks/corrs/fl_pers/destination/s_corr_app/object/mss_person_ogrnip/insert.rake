  ...
  namespace :fl_pers do
    namespace :destination do
      namespace :s_corr_app do
        namespace :object do
          namespace :mss_person_ogrnip do
            ...
            task :insert do |t|
              def query
                ...
                .project([
                  Source.___ids[:link],
                  Source.privates[:ogrnip].as("value"),
                  Arel.sql("#{ Destination::SCorrApp::MSS_PERSON_OGRNIP }").as("object"),
                ])
                ...
                .where(Source.client_types[:name].eq('Физическое лицо')
                  .and(Source.privates[:ogrnip].not_eq(nil))
                )
              end
