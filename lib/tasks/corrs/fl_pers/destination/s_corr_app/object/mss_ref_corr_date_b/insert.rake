  ...
  namespace :fl_pers do
    namespace :destination do
      namespace :s_corr_app do
        namespace :object do
          namespace :mss_ref_corr_date_b do
            ...
            task :insert do |t|
              def query
                ...
                .project([
                  Source.___ids[:link],
                  Source.privates[:date_ogrn].as('value'),
                  Arel.sql("#{ Destination::SCorrApp::MSS_REF_CORR_DATE_B }").as("object"),
                ])
                ...
                .where(Source.client_types[:name].eq('Физическое лицо')
                  .and(Source.privates[:date_ogrn].not_eq(nil))
                )
              end
              ...
              begin
                ...
                Source.execute_query(query.to_sql).each_slice(1000) do |rows|
                  rows.each do |row|
                    insert << {
                      link_up: row["link"],
                      value: row["value"].is_a?(Time) ? row["value"].strftime("%Y%m%d") : nil,
                      object: row["object"],
                    }
                  end
