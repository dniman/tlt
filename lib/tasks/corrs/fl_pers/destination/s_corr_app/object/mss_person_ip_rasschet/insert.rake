  ...
  namespace :fl_pers do
    namespace :destination do
      namespace :s_corr_app do
        namespace :object do
          namespace :mss_person_ip_rasschet do
            ...
            task :insert do |t|
              def query
                ...
                .project([
                  Source.___ids[:link],
                  Source.clients[:account].as("value"),
                  Arel.sql("#{ Destination::SCorrApp::MSS_PERSON_IP_RASSCHET }").as("object"),
                ])
                ...
                .where(Source.client_types[:name].eq('Физическое лицо')
                  .and(Source.clients[:account].not_eq(nil))
                )
              end
