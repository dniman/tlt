  ...
  namespace :fl_pers do
    namespace :destination do
      namespace :s_corr_app do
        namespace :object do
          namespace :column_person_comment do
            ...
            task :insert do |t|
              def query
                ...
                okved =
                  Arel::Nodes::NamedFunction.new(
                    "isnull", [
                      Source.clients[:okved],
                      Arel.sql("''")
                    ]
                  )

                value = Arel::Nodes::Concat.new(
                  Arel::Nodes::Concat.new(Arel.sql("'УФМС: '"), ufms),
                  Arel::Nodes::Concat.new(Arel.sql("'ОКВЭД: '"), okved)
                )
                ...
                .project([
                  Source.___ids[:link],
                  value.as("value"),
                  Arel.sql("#{ Destination::SCorrApp::COLUMN_PERSON_COMMENT }").as("object"),
                ])
                ...
                .join(Source.privufms, Arel::Nodes::OuterJoin).on(Source.privufms[:id].eq(Source.privates[:privufms_id]))
