  ...
  namespace :fl_pers do
    namespace :destination do
      namespace :s_corr_app do
        namespace :object do
          namespace :column_person_insur_num do
            ...
            task :insert do |t|
              def query
                ...
                .project([
                  Source.___ids[:link],
                  Source.privates[:snils].as("value"),
                  Arel.sql("#{ Destination::SCorrApp::COLUMN_PERSON_INSUR_NUM }").as("object"),
                ])
                ...
                .where(Source.client_types[:name].eq('Физическое лицо')
                  .and(Source.privates[:snils].not_eq(nil))
                )
              end
