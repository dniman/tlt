  ...
  namespace :fl_pers do
    namespace :destination do
      namespace :s_corr_app do
        namespace :object do
          namespace :column_person_birthplace do
            ...
            task :insert do |t|
              def query
                ...
                .project([
                  Source.___ids[:link],
                  Source.privates[:bornplace],
                  Arel.sql("#{ Destination::SCorrApp::COLUMN_PERSON_BIRTHPLACE }").as("object"),
                ])
                ...
                .where(Source.client_types[:name].eq('Физическое лицо')
                  .and(Source.privates[:bornplace].not_eq(nil))
                )
              end
