  ...
  namespace :fl_pers do
    namespace :destination do
      namespace :s_corr_app do
        namespace :object do
          namespace :column_person_ufms_code do
            ...
            task :insert do |t|
              def query
                ...
                .project([
                  Source.___ids[:link],
                  Source.privates[:subcode].as("value"),
                  Arel.sql("#{ Destination::SCorrApp::COLUMN_PERSON_UFMS_CODE }").as("object"),
                ])
                ...
                .where(Source.client_types[:name].eq('Физическое лицо')
                  .and(Source.privates[:subcode].not_eq(nil))
                )
              end
