  ...
  namespace :fl_pers do
    namespace :destination do
      namespace :s_corr_app do
        namespace :object do
          namespace :column_person_doc_n do
            ...
            task :insert do |t|
              def query
                ...
                .project([
                  Source.___ids[:link],
                  Source.privates[:pasportno].as("value"),
                  Arel.sql("#{ Destination::SCorrApp::COLUMN_PERSON_DOC_N }").as("object"),
                ])
                ...
                .where(Source.client_types[:name].eq('Физическое лицо')
                  .and(Source.privates[:pasportno].not_eq(nil))
                )
              end
