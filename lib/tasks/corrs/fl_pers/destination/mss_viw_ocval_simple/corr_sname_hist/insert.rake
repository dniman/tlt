          ...
          task :insert do |t|
            ...
            def query
              ...
              subquery.where(
                ...
                .and(Destination.mss_viw_ocval_simple[:varchar].eq(Destination.s_corr[:sname]))
              ...
              varchar =
                Arel::Nodes::Addition.new(
                  Arel::Nodes::Addition.new(Arel.sql("'<varchar>'"), Destination.s_corr[:sname]), Arel.sql("'</varchar>'")
                )
              select_manager = Arel::SelectManager.new
              select_manager.project([
                Destination.mss_objcorr[:link].as("link_app_up"),
                Arel.sql("#{ link_app_prop }").as("link_app_prop"),
                Destination.s_corr[:sname].as("varchar"),
                varchar.as("value"),
