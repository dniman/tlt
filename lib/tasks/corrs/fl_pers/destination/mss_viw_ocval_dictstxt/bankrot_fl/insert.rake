        ...
        namespace :bankrot_fl do
          ...
          task :insert do |t|
            ...
            def query
              ...
              is_bankrot =
                Arel::Nodes::Case.new
                .when(Source.privates[:isbankrot].eq('Y')).then(yes)
              ...
              .where(Source.client_types[:name].eq('Физическое лицо')
                .and(
                  Source.privates[:isbankrot].not_eq(nil)
                  .or(Source.privates[:isbankrot].not_eq('N'))
                )
              )
            end
