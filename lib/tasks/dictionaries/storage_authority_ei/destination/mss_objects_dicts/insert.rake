      ...
      namespace :mss_objects_dicts do
        ...
        task :insert do |t|
          ...
          def query
            ...
            .distinct
            .join(Source.clients).on(Source.clients[:id].eq(Source.intellect[:clients_id]))
            .where(Source.intellect[:clients_id].not_eq(nil))
          end

          begin
            link_dict = Destination.execute_query(link_param_query('STORAGE_AUTHORITY_EI').to_sql).entries.first["link"]
