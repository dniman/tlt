      ...
      namespace :mss_objects_dicts do
        ...
        task :insert do |t|
          ...
          def query
            Source.transptype
            .project([
              Source.transptype[:name],
            ])
            .distinct
            .where(Source.transptype[:name].not_eq(nil))
          end

          begin
            link_dict = Destination.execute_query(link_param_query('TYPE_TRANSPORT').to_sql).entries.first["link"]
