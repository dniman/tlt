          ...
          begin
            ...
            Destination.execute_query(query.to_sql).each_slice(1000) do |rows|
              ...
              sql = <<~SQL
                ...
                from ___ids
                  join ___industries on ___industries.id = ___ids.id and ___ids.table_id = #{ Source::Industries.table_id }
                  join (#{values_list.to_sql}) values_table(#{columns.join(', ')}) on values_table.name = ___industries.name
              SQL
