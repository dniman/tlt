      ...
      namespace :___ids do
        ...
        task :insert do |t|
          def query
            ...
            subquery.where(
              Source.___ids[:id].eq(Source.___industries[:id])
              .and(Source.___ids[:table_id].eq(Source::Industries.table_id))
            )
             
            select_manager = Arel::SelectManager.new
            select_manager.project([
              Arel.sql("table_id = #{Source::Industries.table_id}"),
              Source.___industries[:id],
              Arel::Nodes::NamedFunction.new('newid', []).as("row_id")
            ])
            select_manager.from(Source.___industries)
