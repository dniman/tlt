...
namespace :import do
  ...
  namespace :objects do
    ...
    task :start => [
      ...
      'objects:import',
      'object_references:import:tasks',
    ] do 
  ...
  namespace :charges do
    desc 'Запуск задачи импорта начислений в базу назначения'
    task :start => [
      'set_logger', 
      'source:initialize', 
      'destination:initialize',
      ...
      Rake::Task['import:final_message'].invoke("Импорт начислений в базу назначения завершен.")
    end
  end
  
  # Импорт платежей
  namespace :payments do
    desc 'Запуск задачи импорта платежей в базу назначения'
    task :start => [
      'set_logger', 
      'source:initialize', 
      'destination:initialize',
      ...
      Rake::Task['import:final_message'].invoke("Импорт платежей в базу назначения завершен.")
    end
  end
  
  # Импорт операций движения
  namespace :moving_operations do
    desc 'Запуск задачи импорта операций движения в базу назначения'
    task :start => [
      'set_logger', 
      'source:initialize', 
      'destination:initialize',

      'moving_operations:import',
    ] do 
