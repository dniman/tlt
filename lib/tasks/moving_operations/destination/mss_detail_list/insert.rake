namespace :moving_operations do
  namespace :destination do
    namespace :mss_detail_list do
      task :insert do |t|
        def query
          moving_operations_ids = Source.___ids.alias("moving_operations_ids")
          docroles_ids = Source.___ids.alias("docroles_ids")

          doc_form =
            Arel::Nodes::Case.new
            .when(Source.docset_members[:docrole_id].eq(nil)).then(1)
            .else(Arel::Nodes::Multiplication.new(docroles_ids[:link], -1))

          manager = Arel::SelectManager.new Database.destination_engine
          manager.project(
            moving_operations_ids[:___link_list].as("link_list"),
          ...
          manager.join(Source.___moving_operations).on(Source.___moving_operations[:docset_id].eq(Source.docset_members[:docset_id]))
          manager.join(moving_operations_ids).on(
            moving_operations_ids[:id].eq(Source.___moving_operations[:id])
            .and(moving_operations_ids[:table_id].eq(Source::MovingOperations.table_id))
          )
          ...
          manager.join(docroles_ids).on(
            docroles_ids[:id].eq(Source.docroles[:id])
            .and(docroles_ids[:table_id].eq(Source::Docroles.table_id))
          )
          manager.where(moving_operations_ids[:___link_list].not_eq(nil))
        end
