            ...
            task :insert do |t|
              ...
              def query
                ...
                .where(Source.___ids[:link_type].eq(link_type)
                  .and(Source.states[:amount].not_eq(nil))
                )
              end
              ...
              begin
                sql = ""
                insert = []
                link_param = Destination.execute_query(link_param_query('CAPITAL_SHARE').to_sql).entries.first["link"]
