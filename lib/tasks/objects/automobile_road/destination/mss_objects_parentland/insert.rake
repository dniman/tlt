      ...
      namespace :mss_objects_parentland do
        ...
        task :insert do |t|
          ...
          def query
            ...
            link_automobile_road_type = Destination.execute_query(link_type_query('AUTOMOBILE_ROAD').to_sql).entries.first["link"]
            ...
            build_manager.where(
              Source.___ids[:table_id].eq(Source::Objects.table_id)
                .and(Source.___ids[:link_type].eq(link_automobile_road_type))
            )
            
            automobile_road_manager = Arel::SelectManager.new(Database.source_engine)
            automobile_road_manager.project([
              Source.___ids[:link].as("link_child"),
              Source.enginf[:id],
            ])
            automobile_road_manager.from(Source.___ids)
            automobile_road_manager.join(Source.enginf).on(Source.enginf[:objects_id].eq(Source.___ids[:id]))
            automobile_road_manager.where(
              Source.___ids[:table_id].eq(Source::Objects.table_id)
                .and(Source.___ids[:link_type].eq(link_automobile_road_type))
            )
            ...
            o3 = automobile_road_manager.as('o3')
