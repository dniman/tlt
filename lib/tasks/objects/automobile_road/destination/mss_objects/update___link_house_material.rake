  ...
  namespace :automobile_road do
    namespace :destination do
      namespace :mss_objects do
        ...
        task :update___link_house_material do |t|
          ...
          begin
            ...
            update = [ 
              ___link_house_material: Arel.sql(
                "(
                  select link 
                  from mss_objects_dicts 
                  where name = mss_objects.___house_material
                    and mss_objects_dicts.link_dict = #{ link_param }
                )"
              )
            ]
            where = Arel.sql("mss_objects.___house_material is not null")
