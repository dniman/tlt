  ...
  namespace :land do
    namespace :destination do
      namespace :mss_objects do
        ...
        task :update___link_target_doc_target_doc_id do |t|
          ...
          begin
            ...
            update = [ 
              ___link_target_doc_target_doc_id: Arel.sql(
                "(
                  select link 
                  from mss_objects_dicts 
                  where name = mss_objects.___target_doc_target_doc_id
                    and mss_objects_dicts.link_dict = #{ link_param }
                )"
              )
            ]
            where = Arel.sql("mss_objects.___target_doc_target_doc_id is not null")
