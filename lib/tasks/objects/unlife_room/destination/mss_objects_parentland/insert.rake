      ...
      namespace :mss_objects_parentland do
        ...
        task :insert do |t|
          ...
          def query
            ...
            link_unlife_room_type = Destination.execute_query(link_type_query('UNLIFE_ROOM').to_sql).entries.first["link"]
            ...
            build_manager.where(
              Source.___ids[:table_id].eq(Source::Objects.table_id)
                .and(Source.___ids[:link_type].eq(link_unlife_room_type))
            )
