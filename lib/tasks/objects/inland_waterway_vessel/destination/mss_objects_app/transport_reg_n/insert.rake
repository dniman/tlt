  ...
  namespace :inland_waterway_vessel do
    namespace :destination do
      namespace :mss_objects_app do
        namespace :inland_waterway_vessel_reg_n do
          ...
          task :insert do |t|
            ...
            def query
              ...
              .where(Source.ids[:link_type].eq(link_type)
                .and(Source.transport[:fedno].not_eq(nil))
              )
            end
            ...
            begin
              sql = ""
              insert = []
              link_param = Destination.execute_query(link_param_query('INLAND_WATERWAY_VESSEL_REG_N').to_sql).entries.first["link"]
