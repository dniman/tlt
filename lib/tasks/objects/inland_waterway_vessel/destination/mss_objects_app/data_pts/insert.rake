          ...
          task :insert do |t|
            ...
            def query
              ...
              .where(Source.ids[:link_type].eq(link_type)
                .and(Source.transport[:ps_date].not_eq(nil))
              )
            end
            ...
            begin
              sql = ""
              insert = []
              link_param = Destination.execute_query(link_param_query('DATA_PTS').to_sql).entries.first["link"]
              ...
              sliced_rows.each do |rows|
                rows.each do |row|
                  insert << {
                    ...
                    date: row["ps_date"].strftime("%Y%m%d"),
                  }
                end
