          ...
          namespace :simple_shr_par do
            ...
            task :insert do |t|
              ...
              def query
                ...
                numeric =
                  Arel::Nodes::Case.new()
                  .when(Source.states[:sharesnumber].gt(0)).then(Arel::Nodes::Division.new(Source.states[:nominalcost], Source.states[:sharesnumber]))
                  .else(Arel.sql("0"))
                ...
                .where(Source.___ids[:link_type].eq(link_type)
                  .and(Source.states[:sharesnumber].not_eq(nil))
                )
              end
              ...
              begin
                sql = ""
                insert = []
                link_param = Destination.execute_query(link_param_query('SIMPLE_SHR_PAR').to_sql).entries.first["link"]
