      ...
      namespace :___ids do
        ...
        task :insert do |t|
          ...
          def query
            ...
            select_manager.join(Source.objshares).on(Source.objshares[:objects_id].eq(Source.objects[:id]))
            select_manager.join(Source.sharetypes, Arel::Nodes::OuterJoin).on(Source.sharetypes[:id].eq(Source.objshares[:sharetypes_id]))
            select_manager.where(
              Source.objtypes[:name].eq('Пакеты акций, доли в капитале')
              .and(Source.sharetypes[:name].eq('Акции'))
