      ...
      namespace :mss_objects_parentland do
        ...
        task :insert do |t|
          ...
          def query
            ...
            link_unfinished_type = Destination.execute_query(link_type_query('UNFINISHED').to_sql).entries.first["link"]
            ...
            build_manager.where(
              Source.___ids[:table_id].eq(Source::Objects.table_id)
                .and(Source.___ids[:link_type].eq(link_unfinished_type))
            )
            
            unconstr_manager = Arel::SelectManager.new(Database.source_engine)
            unconstr_manager.project([
              Source.___ids[:link].as("link_child"),
              Source.unconstr[:id],
            ])
            unconstr_manager.from(Source.___ids)
            unconstr_manager.join(Source.unconstr).on(Source.unconstr[:objects_id].eq(Source.___ids[:id]))
            unconstr_manager.where(
              Source.___ids[:table_id].eq(Source::Objects.table_id)
                .and(Source.___ids[:link_type].eq(link_unfinished_type))
            )
            ...
            manager_two.from(Source.links_grounds_unconstr)
            manager_two.join(o1).on(o1[:id].eq(Source.links_grounds_unconstr[:grounds_id]))
            manager_two.join(o3).on(o3[:id].eq(Source.links_grounds_unconstr[:unconstr_id]))
