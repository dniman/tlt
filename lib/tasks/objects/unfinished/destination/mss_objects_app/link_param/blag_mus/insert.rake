            ...
            task :insert do |t|
              ...
              def query
                ...
                select_one = 
                  Source.objects
                  .project([
                    Source.buildings[:bl_garbagebin],
                    Source.ids[:link],
                ...
                select_two = 
                  Source.objects
                  .project([
                    Arel.sql("null as bl_garbagebin"),
                    Source.ids[:link],
                    Source.ids[:link_type],
                  ])
                  .join(Source.objtypes, Arel::Nodes::OuterJoin).on(Source.objtypes[:id].eq(Source.objects[:objtypes_id]))
                ...
                manager.where(union_table[:bl_garbagebin].not_eq(nil))
              end

              begin
                sql = ""
                insert = []
                link_param = Destination.execute_query(link_param_query('BLAG_MUS').to_sql).entries.first["link"]
