            ...
            task :insert do |t|
              ...
              def query
                ...
                select_one = 
                  Source.objects
                  .project([
                    Arel.sql("null as road_ident_number"),
                    Source.ids[:link],
                ...
                select_two = 
                  Source.objects
                  .project([
                    Source.enginf[:road_ident_number],
                    Source.ids[:link],
                    Source.ids[:link_type],
                  ])
                  .join(Source.objtypes, Arel::Nodes::OuterJoin).on(Source.objtypes[:id].eq(Source.objects[:objtypes_id]))
                ...
                manager.where(union_table[:road_ident_number].not_eq(nil))
              end

              begin
                sql = ""
                insert = []
                link_param = Destination.execute_query(link_param_query('ID_AVTODOR').to_sql).entries.first["link"]
                ...
                sliced_rows.each do |rows|
                  rows.each do |row|
                    insert << {
                      ...
                      varchar: row["road_ident_number"]&.strip
                    }
                  end
