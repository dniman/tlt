  ...
  namespace :construction do
    namespace :destination do
      namespace :mss_objects_app do
        namespace :house_date_begin_use do
          ...
          task :insert do |t|
            ...
            begin
              sql = ""
              insert = []
              link_param = Destination.execute_query(link_param_query('HOUSE_DATE_BEGIN_USE').to_sql).entries.first["link"]
              ...
              sliced_rows.each do |rows|
                rows.each do |row|
                  insert << {
                    ...
                    date: row["enter_date"].strftime("%Y%m%d")
                  }
                end
