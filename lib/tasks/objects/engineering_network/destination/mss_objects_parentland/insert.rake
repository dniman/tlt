      ...
      namespace :mss_objects_parentland do
        ...
        task :insert do |t|
          ...
          def query
            ...
            link_engineering_network_type = Destination.execute_query(link_type_query('ENGINEERING_NETWORK').to_sql).entries.first["link"]
            ...
            build_manager.where(
              Source.___ids[:table_id].eq(Source::Objects.table_id)
                .and(Source.___ids[:link_type].eq(link_engineering_network_type))
            )
            
            engineering_network_manager = Arel::SelectManager.new(Database.source_engine)
            engineering_network_manager.project([
              Source.___ids[:link].as("link_child"),
              Source.enginf[:id],
            ])
            engineering_network_manager.from(Source.___ids)
            engineering_network_manager.join(Source.enginf).on(Source.enginf[:objects_id].eq(Source.___ids[:id]))
            engineering_network_manager.where(
              Source.___ids[:table_id].eq(Source::Objects.table_id)
                .and(Source.___ids[:link_type].eq(link_engineering_network_type))
            )
            ...
            o3 = engineering_network_manager.as('o3')
