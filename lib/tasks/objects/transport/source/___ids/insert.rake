      ...
      namespace :___ids do
        ...
        task :insert do |t|
          ...
          def query
            ...
            select_manager.join(Source.transport, Arel::Nodes::OuterJoin).on(Source.transport[:objects_id].eq(Source.objects[:id]))
            select_manager.join(Source.transptype, Arel::Nodes::OuterJoin).on(Source.transptype[:id].eq(Source.transport[:transptype_id]))
            select_manager.where(
              Source.objtypes[:name].eq('Транспортные средства')
