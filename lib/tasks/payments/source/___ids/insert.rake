      ...
      task :insert do |t|
        def query
          ...
          select_manager.project([
            Arel.sql("#{Source::Payments.table_id}").as("table_id"),
            Source.payments[:id],
            Arel::Nodes::NamedFunction.new('newid', []).as("row_id")
          ])
          select_manager.from(Source.payments)
          select_manager.join(Source.movesets).on(Source.movesets[:id].eq(Source.payments[:movesets_id]))
          select_manager.join(Source.movetype, Arel::Nodes::OuterJoin).on(Source.movetype[:id].eq(Source.movesets[:movetype_id]))
          select_manager.where(
            Source.movetype[:name].in([
              'Аренда', 
